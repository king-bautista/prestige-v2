...
class GetUpdateController extends AppBaseController implements GetUpdateControllerInterface
{
    ...
    public function updateContent()
    {
        ...
        // {
            ...
            $this->updateEvents($last_updated_at);
            $this->updateLandmark($last_updated_at);
    ...
    function updateEvents($last_updated_at) 
    {
        $events = Event::on('mysql')->where('updated_at', '>=',$last_updated_at)->get();
        if($events) {
            foreach($events as $event) {

                if($event->image_url)
                    $this->saveMaterial('uploads/media/event/', $event->image_url);

                $data = Event::on('mysql_server')->updateOrCreate(
                    [
                        'id' => $event->id
                    ],
                    [
    ...
    function updateLandmark($last_updated_at) 
    {
        $landmarks = Landmark::on('mysql')->where('updated_at', '>=',$last_updated_at)->get();
        if($landmarks) {
            foreach($landmarks as $landmark) {

                if($landmark->image_url)
                    $this->saveMaterial('uploads/media/landmark/', $landmark->image_url);

                if($landmark->image_thumbnail_url)
                    $this->saveMaterial('uploads/media/landmark/thumbnail/', $landmark->image_thumbnail_url);

                $data = Landmark::on('mysql_server')->updateOrCreate(
                    [
                        'id' => $landmark->id
                    ],
                    [
                        ...
                        'image_thumbnail_url' => $landmark->image_thumbnail_url,
