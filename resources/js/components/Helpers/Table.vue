                    ...
                    <tr v-for="(data, index) in dataTable" v-bind:key="index" class="has-row-actions">
                        <td v-if="checkBox == true">
                            <div v-if="checkedIds.indexOf(data.id) >= 0">
                                <input class="form-check-select" type="checkbox" :value="data.id" name="selected"
                                    checked="checked" @click="getChecked(data.id, $event)">
                            </div>
                            <div v-else>
                                <input class="form-check-select" type="checkbox" :value="data.id" name="selected"
                                    @click="getChecked(data.id, $event)">
                            </div>
                        </td>
...
export default {
    ...
    methods: {
        ...
        sortBy(id, dataFields) {
            var data = dataFields;
            var parsedObjData = JSON.parse(JSON.stringify(dataFields));
            for (var key in parsedObjData) {
                if(key == id){ 
                    if($("#" + key).hasClass('fas fa-sort-up')){ 
                        var class_sort = 'fa-sort-down';
                        this.sort = 'asc'; 
                    }else if ($("#" + key).hasClass('fas fa-sort-down')){ 
                        var class_sort = 'fa-sort-up';
                        this.sort = 'desc';
                    } else{
                        var class_sort = 'fa-sort-down';
                        this.sort = 'asc';
                    ...
                    $("#" + key).removeClass(); 
                    $("#" + key).addClass('fas '+ class_sort);
                }else{
                    $("#" + key).removeClass();
                    $("#" + key).addClass('fas fa-sort');
                }
            }
            this.order = id;
            this.page = 1;
