						...
						<div class="card">
							<div class="card-body">
								<Table :dataFields="dataFields" :dataUrl="dataUrl" :actionButtons="actionButtons"
									:otherButtons="otherButtons" :primaryKey="primaryKey" v-on:addNewRole="addNewRole"
									v-on:editButton="editRole" v-on:modalBatchUpload="modalBatchUpload" v-on:downloadCsv="downloadCsv"
				...
				<div class="modal-content">
					<div class="modal-header">
						...
						<h5 class="modal-title" v-show="edit_record"><i class="fa fa-pencil-square-o"
								aria-hidden="true"></i> Edit Role</h5>
					...
					<div class="modal-body">
						<div class="card-body">
							<div class="form-group row">
								<label for="firstName" class="col-sm-4 col-form-label">Role Name <span
							...
							<div class="form-group row">
								<label for="lastName" class="col-sm-4 col-form-label">Descriptions <span
										class="font-italic text-danger"> *</span></label>
								<div class="col-sm-8">
									<textarea class="form-control" v-model="role.description"
										placeholder="Descriptions"></textarea>
								</div>
							</div>
							<div class="form-group row">
								<label for="firstName" class="col-sm-4 col-form-label">Type <span
										class="font-italic text-danger"> *</span></label>
								<div class="col-sm-8">
									<select class="custom-select" v-model="role.type"
										@change="setPermissions($event.target.value)">
										...
										<table class="table table-hover" style="width:100%">
											...
											<tbody>
												<tr v-for="(module) in role.permissions">
													<td v-bind:class="(module.parent_id) ? 'pl-4' : ''"><i
															:class="module.class_name"></i> {{ module.name }}</td>
													<td class="text-center"><input :class="'view_' + module.str_class"
															type="checkbox" v-model="module.can_view"
															v-on="(!module.parent_id) ? { click: () => checkedAll('view_', module.id) } : { click: () => uncheckedAll('view_', module.parent_id) }">
													</td>
													<td class="text-center"><input :class="'add_' + module.str_class"
															type="checkbox" v-model="module.can_add"
															v-on="(!module.parent_id) ? { click: () => checkedAll('add_', module.id) } : { click: () => uncheckedAll('add_', module.parent_id) }">
													</td>
													<td class="text-center"><input :class="'edit_' + module.str_class"
															type="checkbox" v-model="module.can_edit"
															v-on="(!module.parent_id) ? { click: () => checkedAll('edit_', module.id) } : { click: () => uncheckedAll('edit_', module.parent_id) }">
													</td>
													<td class="text-center"><input :class="'delete_' + module.str_class"
															type="checkbox" v-model="module.can_delete"
															v-on="(!module.parent_id) ? { click: () => checkedAll('delete_', module.id) } : { click: () => uncheckedAll('delete_', module.parent_id) }">
													</td>
												</tr>
											</tbody>
										</table>
					...
					<div class="modal-footer">
						...
						<button type="button" class="btn btn-primary" v-show="add_record" @click="storeRole">Add New
							Role</button>
						<button type="button" class="btn btn-primary" v-show="edit_record" @click="updateRole">Save
							Changes</button>
					</div>
				</div>
			</div>
...
export default {
	...
	methods: {
		...
		storeBatch: function () {
			...
			axios.post('/admin/roles/batch-upload', formData,
				{
					headers: {
		...
		downloadTemplate: function () {
			axios.get('/admin/roles/download-csv-template')
				.then(response => {
					const link = document.createElement('a');
					link.href = response.data.data.filepath;
					link.setAttribute('download', response.data.data.filename); //or any other extension
