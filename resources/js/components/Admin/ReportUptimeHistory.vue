				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<form>
							...
							<div class="form-group row">
								<label for="userName" class="col-sm-4 col-form-label">Date</label>
								<div class="col-sm-8">
...
export default {
	...
	data() {
		return {
			...
			dataFields: {
				...
				name: "Screen Name",
				up_time_date: "Date",
				total_hours: "Total Hours Up",
				opening_hour: "Opening Hour",
				closing_hour: "Closing Hour",
				hours_up: "Max hours up",
				percentage_uptime: "% Uptime",
			},
			primaryKey: "id",
			dataUrl: "/admin/reports/uptime-history/list",
			otherButtons: {
				addNew: {
					title: 'Filter',
					v_on: 'reportModal',
	...
	methods: {
		...
		filterReport: function () {
			...
			var filter_date = this.filter.end_date.replaceAll("/", "-");
			var end_date = new Date(filter_date);
			var priorDate = new Date(new Date(filter_date).setDate(end_date.getDate() - 30)).toISOString().split('T')[0];
			this.filter.start_date = priorDate; //minus 30 days YYYY-MM-DD
			this.filter.end_date = filter_date; //selected YYYY-MM-DD 
		...
		downloadCsv: function () {
			...
			axios.get('/admin/reports/uptime-history/download-csv', { params: { filters: this.filter } })
				.then(response => {
