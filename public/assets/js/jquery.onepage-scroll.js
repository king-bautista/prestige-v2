...
!function($){
  ...
  $.fn.onepage_scroll = function(options){
    ...
    $.fn.moveDown = function() {
      $('.next-section').click();
      var el = $(this)
      index = $(settings.sectionContainer +".current").data("index");// get current for this intance 1, index is now = 1
      current = $(settings.sectionContainer + "[data-index='" + index + "']");// current = section[data-index=1]
      next = $(settings.sectionContainer + "[data-index='" + (index + 1) + "']");// next = section[data-index= 1 + 1]
    ...
    if (ctr == 1){
      $('ul.onepage-pagination').html("");
    }else{
      if (settings.pagination == true) {
      if ($('ul.onepage-pagination.dotstyle.dotstyle-' + settings.dotstyle).length < 1) $("<ul id=dotstyle class='onepage-pagination dotstyle dotstyle-"+settings.dotstyle+"'></ul>").appendTo(".shops-bg,.movie-list-con,.events-bg,.tiles,.prod-tiles,.new_tiles");
